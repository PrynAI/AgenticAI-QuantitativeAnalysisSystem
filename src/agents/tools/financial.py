...
from yahoo finance API. It is designed to be the "Quantiative analyst"
...
#Optional - value can be anything or None. Makes None explicit
from typing import Type,Dict,Any,Optional

#BaseModel -Validates input data
#Field - Field is used to add metadata, constraints, and defaults to model fields.
from pydantic import BaseModel, Field

#BaseTool - abstract base class for defining tools that agents can use in CrewAI
...
class StockAnalysisInput(BaseModel):
    ...
    Enforces that a ticker symbol provided as string.
    """
    ticker:str=Field(...,
                     description="The stock ticker symbol (e.g.'APPL', 'NVDA','MSFT')" )
    
class CompareStocksInput(BaseModel):
    """
    Input schema for the ComparisionStocksTool.
    Requires two distinct tickers for side-side comparison.
    """
    ticker_a:str=Field(...,
...
class FundamentalAnalysisTool(BaseTool):
    """
    A crew AI tool that extracts key fundamental financial metrics for the stock.
